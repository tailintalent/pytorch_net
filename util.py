...
def collate(batch):
    r"""Puts each data field into a tensor with outer dimension batch size, adapted from PyTorch's collate."""
    import re
    ...
    if isinstance(elem, torch.Tensor):
        ...
        Dict = {key: collate([d[key] for d in batch]) for key in elem}
